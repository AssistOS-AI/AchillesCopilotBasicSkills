...
This repository is the baseline catalog of self-contained Anthropic-style skills for Achilles-oriented projects. The canonical documentation entry points are `docs/index.html` and `docs/specsLoader.html?spec=matrix.md`, while the DS specifications under `docs/specs/` are the authoritative contract layer for repository structure, coding style, test organization, runtime defaults, and skill-catalog maintenance.
