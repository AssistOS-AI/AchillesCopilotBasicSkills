...
Each Anthropic-style skill must remain fully self-contained inside its own folder. Descriptors, references, templates, helper modules, and local assets must live within the skill directory. Repository-level conventions from `gamp-specs` and `achilles-specs` govern authoring and documentation, but the runtime behavior of a skill must not depend on importing arbitrary modules from the host project's `src/` tree.
