...
This specification defines the repository bootstrap contract implemented by `gamp_specs`. It covers the documentation surface, DS structure, matrix generation, agent guidance, coding-style anchoring, and synchronization obligations that keep the repository internally consistent.

## Core Content

The repository must maintain both `AGENT.md` and `AGENTS.md`, and the two files must remain synchronized. The HTML documentation must live under `docs/`, use a consistent navigation shell with shared header and footer partials, and include one page for each skill currently present in the repository. The DS specifications must live under `docs/specs/` using the `DS0xx-description.md` convention, with `DS000-vision.md`, `DS001-coding-style.md`, and one DS file per skill.
