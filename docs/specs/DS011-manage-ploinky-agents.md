...
The skill must preserve a fixed set of security invariants. The canonical agent id `agent:<repo>/<agent>` is durable, while route keys and aliases are routing labels that must not change identity. The router is the single public control point, and agent container ports must not be exposed directly. The token families flow in fixed directions: User Session and Guest Session JWTs terminate at the router, an Agent Assertion JWT proves source-agent identity to the router, a Router Request JWT authorizes one concrete internal request to a target AgentServer, and a User Delegation Grant requires an authenticated user rather than a guest. Every internal token binds to the real operation through a recomputed request hash. MCP policy is explicit and fail-closed, where missing policy denies and tags in `mcp-config.json` are only bootstrap defaults. HTTP route access is path-based, uses only `public`, `guest`, or `authenticated`, keeps `public` readonly, and is separate from MCP policy. Chat completions stay non-privileged. Secrets stay separated, so an agent never receives the master key or another agent's secret.
